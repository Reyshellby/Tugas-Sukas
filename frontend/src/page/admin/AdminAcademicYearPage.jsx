import AdminLayout from "./AdminLayout";

export default function AdminAcademicYearPage() {
  return (
    <AdminLayout>
      <h1 className="text-2xl font-bold mb-6">Tahun Ajaran</h1>

      <form className="bg-gray-900 p-6 rounded-xl border border-white/10 space-y-4">
        <input placeholder="Contoh: 2025/2026" className="w-full bg-gray-800 p-2 rounded" />
        <select className="w-full bg-gray-800 p-2 rounded">
          <option>Aktif</option>
          <option>Nonaktif</option>
        </select>
        <button className="bg-indigo-600 px-4 py-2 rounded">Buat Tahun Ajaran</button>
      </form>

      <table className="w-full mt-8 text-sm">
        <thead className="text-gray-400 border-b border-white/10">
          <tr>
            <th className="text-left py-3">Tahun</th>
            <th className="text-left py-3">Status</th>
            <th className="text-left py-3">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr className="border-b border-white/5">
            <td className="py-3">-</td>
            <td className="py-3">-</td>
            <td className="py-3 space-x-2">
              <button className="bg-yellow-600 px-2 py-1 rounded text-xs">Update</button>
              <button className="bg-red-600 px-2 py-1 rounded text-xs">Hapus</button>
            </td>
          </tr>
        </tbody>
      </table>
    </AdminLayout>
  );
}
